...
-- More than 1 insert needed as a table can only store upto 1000 rows in on insert statement
...
/* Based off the 8 sample customers provided in the sample from the subscriptions table, write a brief description about each customer’s onboarding journey -- I did it only for customer 1,13 and 15.
...
WHERE s.customer_id in (1,13,15)
...
/*
  ...
  - Customer 13: started the free trial on 15 Dec 2020 and then subscribed to the basic monthly plan on 22 Dec 2020. 3 months later, customer upgraded to the pro monthly plan
  - Customer 15: started the free trial on 17 Mar 2020 and then subscribed to the pro monthly plan on 24 Marc 2020. However in the following month, on 29 Apr 2020 the customer cancelled the subscription and churned
  				until the paid subscription ended on the 24 May 2020 (as the customer only canceled the plan on the 29 Apr, 4 days after the monthly renewal, the end of billing period will be 24 May 2020)
*/
...
SELECT COUNT(DISTINCT s.customer_id) as nr_customers
FROM foodie_fi.subscriptions s;
...
SELECT 
FORMAT(s.start_date,'yyyy-MM') as year_month
,COUNT(p.plan_id) as monthly_trials
...
SELECT p.plan_id
,p.plan_name
,COUNT(p.plan_name) as nr_events

FROM foodie_fi.subscriptions s
	INNER JOIN foodie_fi.plans p on s.plan_id = p.plan_id and s.start_date >= '2021-01-01'

GROUP BY p.plan_id,p.plan_name
ORDER BY p.plan_id;

-- NOTE: There were no customers on trial plan in 2021. Does it mean that there were no new customers in 2021 or maybe that the 7-week trial option has ended?
	
	  
--B.4 - What is the customer count and percentage of customers who have churned rounded to 1 decimal place?

-- 1: Create a rank per customer and plan subscribed
DROP TABLE IF EXISTS #rank_plan;
SELECT s.customer_id
,s.plan_id
,p.plan_name
,s.start_date
,RANK() OVER(PARTITION BY s.customer_id ORDER BY s.start_date) as rank_plan
...
-- 2: Find the last plan subscribed for each customer
DROP TABLE IF EXISTS #max_rank_plan;
SELECT r.customer_id
,MAX(r.rank_plan) as max_rank_plan
...
-- 3: Create a variable with total customers (used in B1)
DECLARE @total_customers int = (SELECT COUNT(DISTINCT s.customer_id) FROM foodie_fi.subscriptions s);

-- 4: Find the number of customers which the last plan subscribed is churn
SELECT COUNT(r.customer_id) as churn_customers
,ROUND(COUNT(r.customer_id)/CAST(@total_customers as float) * 100,1) as churn_customers_pct

FROM #rank_plan r
	INNER JOIN #max_rank_plan r2 on r.customer_id = r2.customer_id AND r.rank_plan = r2.max_rank_plan

WHERE r.plan_name = 'churn';
...
DECLARE @total_customers2 int = (SELECT COUNT(DISTINCT s.customer_id) FROM foodie_fi.subscriptions s);

SELECT COUNT(*) as nr_customer_chruned_after_trial
,ROUND(COUNT(*)/CAST(@total_customers2 as float) * 100,0) as nr_customer_chruned_after_trial_pct

FROM #rank_plan r

WHERE r.plan_name = 'churn'
	AND r.rank_plan = 2 -- Filter to rank 2 as customers who churned right after trial have a plan ranked as 2
;

--B.6 - What is the number and percentage of customer plans after their initial free trial?
DECLARE @total_customers3 int = (SELECT COUNT(DISTINCT s.customer_id) FROM foodie_fi.subscriptions s);

SELECT r.plan_name
,COUNT(r.customer_id) as nr_subscriptions
,ROUND(COUNT(r.customer_id)/CAST(@total_customers3 as float) * 100,1) as pct_subscriptions
...
DECLARE @total_customers_2020 int = (SELECT COUNT(DISTINCT s.customer_id) FROM foodie_fi.subscriptions s
...
with rank_plan as (
	...
	,max_rank_plan as (
	SELECT customer_id
	,MAX(plan_rank) as max_plan_rank
...
SELECT r.plan_id
,r.plan_name
,COUNT(r.customer_id) as nr_customers
,COUNT(r.customer_id)/CAST(@total_customers_2020 as float) * 100 as pct_customers

FROM rank_plan r 
	INNER JOIN max_rank_plan r2 on r.customer_id = r2.customer_id AND r.plan_rank = r2.max_plan_rank

GROUP BY r.plan_name,r.plan_id
ORDER BY r.plan_id;

--B.8 - How many customers have upgraded to an annual plan in 2020?
SELECT COUNT(s.customer_id) as nr_customers
...
DROP TABLE IF EXISTS #annual_plan_date;
...
SELECT s.customer_id
,MIN(s.start_date) as annual_plan_date
INTO #annual_plan_date
...
SELECT AVG(DATEDIFF(day,j.join_date,a.annual_plan_date)) as avg_days_to_annual_plan

FROM #join_date j
	INNER JOIN #annual_plan_date a on j.customer_id = a.customer_id
;

--B.10 - Can you further breakdown this average value into 30 day periods (i.e. 0-30 days, 31-60 days etc)
SELECT 
CASE 
	WHEN DATEDIFF(day,j.join_date,a.annual_plan_date) between 0 and 30   THEN '0-30 days'
	WHEN DATEDIFF(day,j.join_date,a.annual_plan_date) between 31 and 60  THEN '31-60 days' 
	WHEN DATEDIFF(day,j.join_date,a.annual_plan_date) between 61 and 90  THEN '61-90 days'
	WHEN DATEDIFF(day,j.join_date,a.annual_plan_date) between 91 and 120 THEN '91-120 days'
	ELSE 'More than 120 days'
END AS period_breakdown
,COUNT(j.customer_id) as nr_customers
...
with next_plan as (
	SELECT s.customer_id
	,p.plan_name as plan_subscribed
	,LEAD(p.plan_name,1) OVER (PARTITION BY s.customer_id ORDER BY s.start_date) as next_plan_subscribed  
...
SELECT *
FROM next_plan
WHERE plan_subscribed = 'pro monthly'
	AND next_plan_subscribed  = 'basic monthly';

-- No customer has downgraded from a pro monthly plan to a basic monthly plan in 2020
...
-- 1: Push all the needed from the subscriptions table plus a lead function to find the next_date for each row
with lead_cte as (
				   SELECT s.customer_id
				   ,p.plan_id
				   ,p.plan_name
				   ...
				   WHERE 1 = 1
						AND p.plan_id <> 0 -- Exclude trial plans because they are free
)
...
,reviewed_cte as (
					...
					,CASE
						...
						WHEN next_date IS NULL OR LEFT(next_date,4) = 2021 THEN '2020-12-31' -- payment_date will be an ongoing series until the end of 2020 (requested). 
						ELSE next_date														 -- NULL values indicate monthly plans (id: 1,2) that remain active/unchanged in the database
					END AS next_date
					,price
					
					FROM lead_cte
)
-- 3: Create a recursive CTE to generate the payments series as Microsoft SQL Server doesn't have a build in function to generate series
,recursive_cte as (
					SELECT *
					FROM reviewed_cte
					...
					FROM recursive_cte
					WHERE start_date < DATEADD(month,-1,next_date) -- To create a new row, the recursive query will use the output from the previous iteration (n-1) as input for the new iteration
					)                                              -- Ex: to create a new row for id_customer: 1 where start date: 2020-12-08 the query used as input the output from the last row created: 2020-11-08
																   -- So, in order to create this new row, it will check for the termination condition where 2020-11-08 < 2020-11-31: as it is TRUE the new row is created)
																   -- Due to the recursive logic we need to consider the next_date 1 month behind to match n-1 logic

-- 4: In the final SELECT statement we need to consider if a customer upgradates from a basic monthly to a pro monthly, we need to deduct the different between the current plan and the new plan
SELECT customer_id
,plan_id
,plan_name
...
SELECT year_month
,nr_customers as current_month_customers
,LAG(nr_customers,1) OVER(ORDER BY year_month) as previous_month_customers
,ROUND((CAST(nr_customers as float) - LAG(nr_customers,1) OVER(ORDER BY year_month))/LAG(nr_customers,1) OVER(ORDER BY year_month) * 100,1) as pct_growth
